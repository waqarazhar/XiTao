...
// this is an MPI+XiTAO implementation. For now, halo-exchange is ignored because it doesn't contribute to the interference analysis
int main( int argc, char *argv[] )
{
    ...
    if(mpirank == 0) print_params(&param);

   if(getenv("GOTAO_NTHREADS"))
        nthreads = atoi(getenv("GOTAO_NTHREADS"));
   else 
   {
      if(mpirank == 0) fprintf(stderr, "Error, For the MPI+XiTAO version, GOTAO_NTHREADS env must be set.\n\n");
      return 1;
    }
   ...
   if(getenv("GOTAO_THREAD_BASE")) 
    ...
    np = ceil(sqrt(ceildiv(np * np, mpisize)));    
    if(mpirank == 0)
    ...
    gotao_init_hw(nthreads, nthreads * mpirank, -1, mpirank != 0);
